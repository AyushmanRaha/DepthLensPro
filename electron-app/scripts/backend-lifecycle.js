...
async function smoke(){ let code=0; for(const [ep,to] of [['/live',3000],['/ready',7000],['/devices',5000],['/health',8000]]){ const r=await request(ep,to); console.log(`${ep}: ${JSON.stringify(r)}`); if(r.error||!r.status||r.status>=400||r.empty){ code=1; if(r.connected&&r.error) console.error(`${ep} failure: TCP connects but receives 0 bytes / no HTTP response.`); } } const pid=listeningPid(); console.log(`lsof/netstat PID: ${pid||'none'}`); for(const f of pidFiles()){ console.log(`${f.pidPath}: ${readFileSafe(f.pidPath)||'(missing)'}`); console.log(`${f.metaPath}: ${readFileSafe(f.metaPath)||'(missing)'}`); } return code; }
