"""Serve depth-estimation inference endpoints and runtime diagnostics."""
...
def _get_apple_chip() -> str | None:
    ...
    Returns:
        Apple chip label, or None when unavailable.
    """
...
def _mps_available() -> bool:
    ...
    Returns:
        True when PyTorch has MPS support and the host can use it.
    """
    return (
...
def _available_devices() -> dict:
    ...
    CUDA, MPS, and XPU are classified as GPU backends because PyTorch does not
    expose Apple ANE or Intel AI Boost as torch.device targets.

    Returns:
        Mapping of device keys to device metadata.
    """
    cpu_name = (
        platform.processor() or os.environ.get("PROCESSOR_IDENTIFIER") or "System CPU"
    )
    devs = {
        "cpu": {
            "name": f"CPU · {cpu_name}",
    ...
    # torch.xpu targets Intel GPU silicon, not Intel AI Boost NPU.
    ...
    # MPS uses Apple Silicon GPU cores, not the Apple Neural Engine.
    if _mps_available():
...
def _default_device_key() -> str:
    ...
    CUDA is preferred for PyTorch maturity, followed by MPS, XPU, and CPU.

    Returns:
        Device key for the selected compute target.
    """
    ...
    if "mps" in devs:
        return "mps"
    # Prefer the first Intel GPU when CUDA and MPS are unavailable.
...
def _acceleration_checks(devs: dict) -> dict:
    ...
    Returns:
        Availability and operational status for each supported accelerator type.
    """
    checks = {}
...
async def lifespan(app: FastAPI):
    ...
    try:
        # Warm the default model to keep the first inference request predictable.
        _load_model("MiDaS_small", best)
        log.info(f"✅ MiDaS_small pre-loaded on {best}")
    except Exception as e:
        log.warning(f"⚠️  Pre-load on {best} skipped: {e}")
        # Preserve startup readiness when accelerator model loading fails.
        try:
...
def _load_model(model_name: str, device_str: str):
    ...
    Args:
        model_name: Supported MiDaS model identifier.
        device_str: Concrete torch device key; callers resolve "auto" first.

    Returns:
        Cached model/device tuple and transform callable.
    ...
    # MPS remains most reliable with float32 model weights.
    if device.type == "mps":
        model = model.float()
...
def _fhash(raw: bytes, model: str, cmap: str, dev: str) -> str:
    return hashlib.sha1(
        f"{model}:{cmap}:{dev}:{hashlib.md5(raw).hexdigest()}".encode()
    ).hexdigest()
...
async def health():
    ...
    # Treat acceleration as ready only after a runtime tensor probe succeeds.
    ...
    return {
        ...
        # PyTorch exposes GPU backends here; it does not expose an NPU device.
...
async def estimate(
    ...
    # Resolve once so model cache keys always use concrete device strings.
...
async def batch(
    ...
    for f in files:
        try:
            ...
            ck = _fhash(raw, model, colormap, resolved)
            if ck in CACHE:
                results.append({**CACHE[ck], "cached": True})
                continue
