...
from backend.model_registry import MODEL_REGISTRY, DEFAULT_MODEL_DIR, resolve_onnx_path
...
class ModelAssetsUnavailableError(RuntimeError):
    ...
    def __init__(self, message: str | None = None, *, status: dict[str, Any] | None = None, cause: BaseException | None = None) -> None:
...
def downloads_disabled() -> bool:
    return os.getenv("DEPTHLENS_DISABLE_MODEL_DOWNLOADS", "").strip().lower() in {"1", "true", "yes", "on"}
...
def _checkpoint_summary(cache: Path) -> dict[str, Any]:
    ...
    for model_id, hints in MODEL_TO_CHECKPOINT_HINTS.items():
        matches = [f for f in files if f["size_bytes"] > 0 and any(h in f["name"].lower() for h in hints)]
        by_model[model_id] = {"ready": bool(matches), "files": matches}
    return {"path": os.fspath(checkpoints), "exists": checkpoints.is_dir(), "files": files, "by_model": by_model}
...
def _midas_repo(cache: Path) -> dict[str, Any]:
    ...
    for repo in candidates:
        if (repo / "hubconf.py").is_file() and ((repo / "midas").is_dir() or (repo / "MiDaS").is_dir()):
            valid.append(repo)
    return {"cached": bool(valid), "candidates": [os.fspath(p) for p in candidates], "valid_repos": [os.fspath(p) for p in valid]}


def onnx_asset_status() -> dict[str, Any]:
    models = {mid: resolve_onnx_path(mid) for mid in MIDAS_MODEL_IDS}
    any_ready = any(item.get("exists") and int(item.get("size_bytes") or 0) > 0 for item in models.values())
    all_ready = all(item.get("exists") and int(item.get("size_bytes") or 0) > 0 for item in models.values())
    return {"models": models, "onnx_any_ready": any_ready, "onnx_all_ready": all_ready}


def inspect_model_assets(*, required_models: tuple[str, ...] = MIDAS_MODEL_IDS, cache_root: Path | None = None) -> dict[str, Any]:
    ...
    recommended = None if pytorch_ready else "Run npm run setup:<platform> and rebuild the app. For ONNX builds run npm run setup:<platform>:onnx."
    return {
...
def model_assets_error_payload(status: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "action": status.get("recommended_action") or "Run npm run setup:<platform> and rebuild the app.",
        "torch_home": status.get("pytorch_hub_cache_path"),
        "expected_cache": status.get("pytorch_hub_cache_path"),
        "standard_build_note": "ONNX is optional; PyTorch MiDaS assets are required for standard builds.",
        "details": status,
    }
...
def classify_torch_hub_failure(exc: BaseException) -> bool:
    ...
    return downloads_disabled() or any(token in text for token in ("no such file", "not found", "unable to find", "urlopen", "connection", "permission", "read-only", "readonly", "checkpoints"))
