/**
 * Coordinates the DepthLens Pro renderer state, inference requests, and UI updates.
 */
"use strict";

// ── Config and state ─────────────────────────────────────────────
// Electron supplies the packaged backend URL; browser development uses localhost.
...
const state = {
  files: [], // Queue entries retain source file, thumbnail, status, and result.
  results: [], // Completed inference payloads.
  session: {
    total: 0,
    cached: 0,
    errors: 0,
  ...
  lb: { current: null },
  abort: null, // AbortController for the active batch request.
  compareAbort: null,
  compareFile: null,
  devices: {},
  primaryDevice: "cpu",
...
function loadPrefs() {
  try {
    ...
    // Restore the saved device only after the backend reports valid options.
    window._savedDevice = p.device;
  } catch {}
}

// Persist user-facing controls whenever they change.
...
(function bgCanvas() {
  ...
  function draw() {
    ...
    for (let i = 0; i < pts.length; i++) {
      ...
      ctx.fillStyle = `rgba(0,200,255,${p.a * 0.55})`;
      ctx.fill();
    }
...
const COMPARE_METRICS = [
  ...
  {
    key: "dynamic_range",
    label: "Dynamic Range (bits)",
    source: "metrics",
    better: "higher",
    fmt: (v) => `${Number(v).toFixed(2)} bits`,
  },
];
...
function renderCompareSummary(results) {
  ...
  summaryMetrics.forEach((spec) => {
    const valid = results
      .map((r) => ({
        model: r.model,
        value: Number(compareMetricValue(r, spec)),
      }))
    ...
    const sorted = [...valid].sort((a, b) =>
      spec.better === "higher" ? b.value - a.value : a.value - b.value,
    );
...
function renderCompareChart(results, metricKey = state.compareView.metricKey) {
  const metric =
    COMPARE_METRICS.find((m) => m.key === metricKey) || COMPARE_METRICS[0];
      ...
      datasets: [
        {
          ...
          backgroundColor: values.map((v) =>
            v === null ? "rgba(127,140,153,.45)" : "rgba(0,200,255,.55)",
          ),
          borderColor: values.map((v) => (v === null ? "#5e6f81" : "#00c8ff")),
          borderWidth: 1.5,
          borderRadius: 4,
        },
      ],
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            label: (c) =>
              c.raw === null ? "Not available" : metric.fmt(c.raw),
          },
        },
      },
      scales: {
...
function initCompareControls() {
  el.compareMetricSelect.innerHTML = COMPARE_METRICS.map(
  ...
  el.compareChartToggle.addEventListener("click", () => {
    ...
    el.compareChartToggle.textContent = state.compareView.open
      ? "Hide Graph"
      : "Show Graph";
    el.compareChartToggle.setAttribute(
      "aria-expanded",
      String(state.compareView.open),
    );
  });
}

// ── Server health and device list ───────────────────────────
// Status reflects the local FastAPI inference engine, not a remote service.
...
function classifyKinds(info = {}) {
  const set = new Set(
    (Array.isArray(info.compute_classes)
...
async function checkHealth() {
  ...
  try {
    ...
    // Surface the backend-selected accelerator in the header.
    ...
    let badge = "CPU";
    if (info?.type === "cuda")
      badge = `GPU: ${info.name} (${info.memory_gb} GB)`;
    ...
    setStatus(
      "online",
      "Depth Engine: Online",
      `PyTorch ${data.torch_version} · ${primary} · ${accelText}`,
      badge,
    );

    // Rebuild selector options from backend capabilities.
    await loadDevices(devs, primary);
    updateDeviceInfoBanner(`${badge} · ${accelText}`);

    if (!data.acceleration_ok) {
      toast(
        "Acceleration check warning: one or more accelerator backends are unavailable.",
        "warning",
      );
    }
    return true;
...
function autoDeviceLabel(devs, primary) {
  ...
  if (preferred.type === "cuda")
    return `Prefers GPU · ${preferred.name || primary}`;
  if (preferred.type === "mps")
    return `Prefers Apple acceleration · ${preferred.name || primary}`;
  if ((preferred.compute_classes || []).includes("npu"))
    return `Prefers NPU · ${preferred.name || primary}`;
...
async function loadDevices(devs, primaryFromHealth = null) {
  ...
  state.devices = devs;
  const primary =
    primaryFromHealth && devs[primaryFromHealth]
      ? primaryFromHealth
      : devs.mps
        ? "mps"
        : keys.includes("cuda:0")
          ? "cuda:0"
          : keys[0];
  state.primaryDevice = primary;
  ...
  const kindsPresent = {
    ...
    gpu: withKinds.some(
      (d) =>
        d.kinds.includes("gpu") ||
        d.kinds.includes("cuda") ||
        d.kinds.includes("mps"),
    ),
    npu: withKinds.some(
      (d) => d.kinds.includes("npu") || d.kinds.includes("ane"),
    ),
  };
  ...
  if (!filters.find((f) => f.id === state.deviceFilter))
  ...
  filters.forEach((f) => {
    ...
    b.addEventListener("click", () => {
      ...
      [...el.deviceTypeToggle.children].forEach((ch) =>
  ...
  // Keep comparison devices aligned with the primary selector.
...
function renderDeviceSelector(deviceEntries, primary, saved, devs) {
  ...
  deviceEntries.forEach(({ key, info, kinds }) => {
    ...
    const classLabel = [
      kinds.includes("gpu") ? "GPU" : null,
      kinds.includes("npu") ? "NPU" : null,
      kinds.includes("cpu") ? "CPU" : null,
    ]
      .filter(Boolean)
      .join("/");
    const sub = isCuda
      ? `CUDA · ${info.memory_gb} GB VRAM · ${classLabel}`
      : isMps
        ? `Apple ${info.chip || "Silicon"} · Metal + Neural Engine · ${classLabel}`
        : isXpu
          ? `XPU backend · ${classLabel}`
  ...
  // Keep toggle state consistent with the selected device class.
  $$(".device-opt input", el.deviceSelector).forEach((inp) => {
    inp.addEventListener("change", () => {
      $$(".device-opt", el.deviceSelector).forEach((l) =>
...
// ── File handling ─────────────────────────────────────────
// Native label binding remains the safest path to open the file picker.
const ALLOWED = /^image\//;
...
function addFiles(list) {
  ...
  for (const file of list) {
    if (!ALLOWED.test(file.type)) {
      toast(`Skipped "${file.name}" — not an image`, "warning");
      continue;
    }
    if (file.size > 20 * 1024 * 1024) {
      toast(`Skipped "${file.name}" — exceeds 20 MB`, "warning");
      continue;
    }
    if (
      state.files.some(
...
// File input handles both label clicks and drag-drop transfers.
el.fileInput.addEventListener("change", () => {
  addFiles(el.fileInput.files);
  el.fileInput.value = "";
});

// Drop zone mirrors native file input behavior.
...
// Preserve browse behavior when layout changes intercept label clicks.
el.dropZone.addEventListener("click", (e) => {
  if (e.target.closest("#fileInput")) return;
  el.fileInput.click();
});
// Support keyboard activation for the custom drop target.
...
// ── Inference ───────────────────────────────────────────────
function selModel() {
  return $('input[name="model"]:checked')?.value || "MiDaS_small";
}
function selCmap() {
  return $('input[name="colormap"]:checked')?.value || "inferno";
}
function selDevice() {
  return (
    $('input[name="device"]:checked')?.value || state.primaryDevice || "cpu"
  );
}
...
function setProgress(pct, status, eta, currentFile, countStr) {
  ...
  if (currentFile !== undefined)
    el.progressCurrentFile.textContent = currentFile;
  if (countStr !== undefined) el.progressItemCount.textContent = countStr;
}

function timingKey(model, device) {
  return `${model}::${device || "cpu"}`;
}
function getEstimate(mode, model, device) {
  return (
    state.timing[mode]?.[timingKey(model, device)] ||
    state.timing[mode]?.default ||
    1300
  );
}
function updateEstimate(mode, model, device, ms) {
  ...
  const vals = Object.values(state.timing[mode]).filter(
    (v) => typeof v === "number",
  );
  if (vals.length)
    state.timing[mode].default = vals.reduce((a, b) => a + b, 0) / vals.length;
}
...
async function runBatch() {
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    const tick = setInterval(() => {
      ...
      setProgress(
        pct,
        `Processing ${i + 1} of ${pending.length}…`,
        `ETA: ${fmtDuration(rem)}`,
        esc(entry.file.name),
        `${i + 1} / ${pending.length}`,
      );
    ...
    try {
      ...
      if (err.name === "AbortError") {
        setFileSt(entry.id, "pending", "Cancelled");
        break;
      }
      entry.status = "error";
  ...
  setProgress(
    100,
    `Done — ${done} image${done !== 1 ? "s" : ""} in ${fmtDuration(elapsed)}`,
    "",
  );
...
function appendGalleryItem(r) {
  ...
  item.addEventListener("keydown", (e) => {
    if (e.key === "Enter" || e.key === " ") openLightbox(r);
  });
...
const METRIC_GROUPS = [
    ...
    metrics: [
      {
        ...
        unit: "",
        desc: "Average magnitude of deviation from depth mean. Lower = more uniform depth field. High MAE suggests a wide depth range (which can be desirable).",
        needsGT: false,
      },
      {
        key: "rmse",
        ...
        unit: "",
        desc: "Square-root of mean squared deviation from depth mean. Penalises large outliers more than MAE.",
        needsGT: false,
      },
      {
        key: "log_rmse",
        label: "Log RMSE",
        unit: "",
        desc: "RMSE computed in log-depth space — more sensitive to relative errors at small depth values.",
        needsGT: false,
      },
      {
        key: "abs_rel",
        label: "Absolute Relative Error (Abs Rel)",
        unit: "",
        desc: "Mean of |pred−GT|/GT. Standard MDE benchmark metric. Requires ground-truth depth.",
        needsGT: true,
      },
      {
        key: "sq_rel",
        label: "Squared Relative Error (Sq Rel)",
        unit: "",
        desc: "Mean of (pred−GT)²/GT. Penalises large relative errors. Requires ground-truth depth.",
        needsGT: true,
      },
    ],
  },
    ...
    metrics: [
      {
        ...
        label: "δ < 1.25¹",
        unit: "%",
        desc: "Fraction of pixels within 25% scale of ground truth. Typical MDE benchmark metric. Requires GT.",
        needsGT: true,
      },
      {
        key: "delta_2",
        label: "δ < 1.25²",
        unit: "%",
        desc: "Looser threshold (56%). Requires GT.",
        needsGT: true,
      },
      {
        key: "delta_3",
        label: "δ < 1.25³",
        unit: "%",
        desc: "Loosest threshold (95%). Requires GT.",
        needsGT: true,
      },
    ],
  },
  {
    ...
    metrics: [
      {
        key: "silog",
        label: "SILog (Scale-Invariant Log Error)",
        unit: "",
        desc: "Measures log-depth variance after removing global scale. Lower = better structure preservation. Commonly used on KITTI / NYU benchmarks.",
        needsGT: false,
      },
      {
        key: "dynamic_range",
        label: "Dynamic Range",
        unit: " bits",
        desc: "Log₂ ratio of max/min non-zero depth. Larger = more depth variation captured. ~8–14 bits is typical for natural scenes.",
        needsGT: false,
      },
      {
        key: "entropy",
        ...
        unit: " bits",
        desc: "Entropy of the depth histogram (bits). Higher = more uniformly distributed depth values. Low entropy indicates most pixels cluster at a single depth.",
        needsGT: false,
      },
      {
        key: "coverage",
        ...
        unit: "%",
        desc: "Fraction of histogram bins with ≥1% of peak count. Higher = depth values spread across the full range.",
        needsGT: false,
        pct: true,
      },
    ],
    ...
    metrics: [
      {
        ...
        unit: "",
        desc: "Compares depth map structure to grayscale input. Range 0–1; higher means the depth map preserves the luminance structure of the scene (edges, textures).",
        needsGT: false,
      },
      {
        key: "gradient_mean",
        label: "Gradient Mean",
        unit: "",
        desc: "Mean Sobel gradient magnitude over the depth map. Higher = more depth edges / transitions. Low values indicate blurry or featureless depth maps.",
        needsGT: false,
      },
      {
        key: "gradient_std",
        label: "Gradient Std Dev",
        unit: "",
        desc: "Variation in gradient strength. High std means some regions have sharp edges while others are smooth.",
        needsGT: false,
      },
      {
        key: "gradient_error",
        ...
        unit: "",
        desc: "Proxy for edge detail in the depth map (mean gradient). Higher = more preserved depth boundaries.",
        needsGT: false,
      },
      {
        key: "edge_density",
        label: "Edge Density",
        unit: "%",
        desc: "Fraction of pixels with gradient > mean+std. Indicates how richly detailed the depth edges are.",
        needsGT: false,
        pct: true,
      },
      {
        key: "surface_normal_error",
        label: "Surface Normal Error",
        unit: "",
        desc: "Requires ground-truth normals derived from GT depth. Not computable without GT.",
        needsGT: true,
      },
    ],
  },
    ...
    metrics: [
      {
        ...
        unit: " dB",
        desc: "Signal quality of the depth map relative to its own mean. Higher = less self-noise. Computed in depth space; not directly comparable to image PSNR.",
        needsGT: false,
      },
      {
        key: "lpips",
        label: "LPIPS (Perceptual Similarity)",
        unit: "",
        desc: "Learned perceptual metric based on deep features. Requires a reference depth map or image. Not computable without GT.",
        needsGT: true,
      },
    ],
  },
    ...
    metrics: [
      {
        ...
        label: "Ordinal Error",
        unit: "",
        desc: "Fraction of pixel pairs where the relative ordering of pred depth disagrees with GT. Key metric for monocular methods. Requires GT.",
        needsGT: true,
      },
      {
        key: "abs_rel",
        label: "Relative Depth Accuracy",
        unit: "",
        desc: "Accuracy of relative depth relationships. Requires GT reference depth map.",
        needsGT: true,
      },
    ],
  },
...
function valColor(key, val) {
  // Use stable thresholds for metrics with known quality direction.
  ...
  METRIC_GROUPS.forEach((group, gi) => {
    ...
    if (group.note) {
      ...
      noteEl.style.cssText =
        "font-family:var(--ff-mono);font-size:.6rem;color:var(--text-dim);margin-bottom:.4rem;line-height:1.4;";
      noteEl.textContent = group.note;
      content.appendChild(noteEl);
    }

    // Prevent duplicate aliases from rendering within the same group.
    const seen = new Set();
    group.metrics.forEach((m) => {
      ...
      let valText, cls;
      if (m.needsGT && isNull) {
        valText = "N/A (needs GT)";
        cls = "na";
      } else if (isNull) {
        valText = "—";
        cls = "na";
      } else if (m.pct) {
        valText = `${(raw * 100).toFixed(1)}%`;
        cls = valColor(m.key, raw);
      } else {
        valText = `${raw}${m.unit || ""}`;
        cls = valColor(m.key, raw);
      }
    ...
    // Open the first metric group to give the panel an immediate anchor.
...
function toggleAccordion(div) {
  ...
  div
    .querySelector(".metric-group-header")
...
function openLightbox(r) {
  ...
  // Summarize request metadata next to the lightbox controls.
  el.lbTags.innerHTML = [
    r.model?.replace("MiDaS_", "").replace("DPT_", "DPT "),
    r.colormap,
    r.device_used,
...
function closeLightbox() {
  if (
    el.lightboxBackdrop.hidden ||
    el.lightboxBackdrop.classList.contains("is-closing")
  )
    return;
...
el.lbDlDepth.addEventListener("click", () => {
  const r = state.lb.current;
  if (r) dlB64(`depth_${r.filename}`, r.depth_map);
});
el.lbDlGray.addEventListener("click", () => {
  const r = state.lb.current;
  if (r) dlB64(`gray_${r.filename}`, r.grayscale);
});
...
el.compareDropZone.addEventListener("dragover", (e) => {
  e.preventDefault();
  el.compareDropZone.classList.add("drag-over");
});
el.compareDropZone.addEventListener("dragleave", (e) => {
  if (!el.compareDropZone.contains(e.relatedTarget))
...
el.compareDropZone.addEventListener("drop", (e) => {
  ...
  if (f?.type.startsWith("image/")) {
    state.compareFile = f;
    el.compareFileName.textContent = f.name;
    el.compareRunBtn.disabled = false;
    toast(`Loaded: ${f.name}`);
  }
});
...
el.compareCancelBtn.addEventListener("click", () => {
  state.compareAbort?.abort();
  toast("Comparison cancelled", "warning");
});
...
async function runComparison() {
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    try {
      const r = await inferOne(
        state.compareFile,
        model,
        cmap,
        device,
        state.compareAbort.signal,
      );
      clearInterval(tick);
  ...
  el.compareProgressText.textContent = state.compareAbort.signal.aborted
    ? "Cancelled"
    : "Done!";
  el.compareProgressEta.textContent = `Total: ${fmtDuration(Date.now() - t0)}`;
  setTimeout(() => {
    el.compareProgressBlock.hidden = true;
  }, 2500);
  el.compareRunBtn.disabled = false;
