...
function prettyEngineName(engine) { const e=String(engine||"").toLowerCase(); if (e==="onnxruntime"||e==="onnx"||e==="onnxruntime_prefer") return "Prefer ONNX Runtime"; if (e==="onnxruntime_strict") return "Force ONNX Runtime Strict"; if (e==="pytorch") return "PyTorch"; if (e==="auto") return "Auto"; if (e==="cache") return "Cached"; return e ? e.replace(/_/g," ") : "—"; }
function formatEngineStatus(result) { const req=result?.engine_requested||"auto", used=result?.engine_used; const lat=Number.isFinite(Number(result?.latency_ms)) ? `${result.latency_ms} ms` : "—"; const reason=result?.onnx_failure_reason||result?.fallback_reason||result?.onnx_diagnostics?.exception_message||result?.engine_selection?.onnx_failure_reason||"session failed"; if (used==="onnxruntime" && (result?.provider_fallback_used || result?.engine_selection?.provider_fallback_used)) return `${lat} · ONNX Runtime · CPU provider fallback`; if (String(req).includes("strict") && !used) return `${lat} · ONNX Runtime failed (${reason})`; if (result?.fallback_used) return `${lat} · ONNX execution failed → ${prettyEngineName(used)} fallback (${reason})`; if (used==="cache") { const selected=result?.engine_selection?.selected_engine; return `${lat} · Cached${selected&&selected!=="cache" ? ` · ${prettyEngineName(selected)}` : ""}`; } if (req==="auto" && used) return `${lat} · Auto → ${prettyEngineName(used)}`; return `${lat} · ${prettyEngineName(used||req)}`; }
