...
function formatEngineStatus(result) { const req=result?.engine_requested||"auto", used=result?.engine_used; const lat=Number.isFinite(Number(result?.latency_ms)) ? `${result.latency_ms} ms` : "—"; const reason=result?.onnx_failure_reason||result?.fallback_reason||result?.engine_selection?.onnx_failure_reason||"session failed"; if (used==="onnxruntime" && (result?.provider_fallback_used || result?.engine_selection?.provider_fallback_used)) return `${lat} · ONNX Runtime · CPU provider fallback`; if (result?.fallback_used) return `${lat} · ONNX session failed → ${prettyEngineName(used)} fallback (${reason})`; if (used==="cache") { const selected=result?.engine_selection?.selected_engine; return `${lat} · Cached${selected&&selected!=="cache" ? ` · ${prettyEngineName(selected)}` : ""}`; } if (req==="auto" && used) return `${lat} · Auto → ${prettyEngineName(used)}`; return `${lat} · ${prettyEngineName(used||req)}`; }
